...
check('api() retries once on a transient drop but NOT on auth or timeout', /async function api\(action, params\)\{[\s\S]*catch\(err\)\{[\s\S]*if\(err && \(err\.kind==="auth" \|\| err\.kind==="timeout"\)\) throw err;[\s\S]*return await apiCall\(action, params\);/.test(html), null);
...
// Backpressure must stay SILENT: it changes the poll interval, never the freshness
// wording. The owner asked to keep the plain "Up to date / Updating…" indicator, so
// the "server busy"/"busier than usual" note must NOT be appended to the line.
check('backpressure is silent — no "busy" text on the freshness line', !/server busy|busier than usual/.test(html) && !/function reflectLoadLevel\(/.test(html), null);
