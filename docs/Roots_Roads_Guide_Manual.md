# Roots & Roads — Guide Manual

## Logging in

1. On your phone, open **rootsandroadsbcn.com/guide** (link also at the bottom
   of the main website).
2. Sign in with the email and password management gave you.
3. You stay signed in for the day; log in again if it asks.

## Finding your tours

- **My tours**: every upcoming tour assigned to you, grouped by day. TODAY and
  TOMORROW are badged.
- **All guides / All tours**: the full schedule — you can see which guide runs
  which tour this week. Reservation details (names, phones) are only visible
  for YOUR tours. Managers see and can edit everything.

## Reading a tour card

- Time + language circle (EN / DE / ES).
- **Private** pill = private tour: only that group, at the exact time shown.
- Numbers like `0 / 2` = checked-in / booked adults.
- `2+3` = **2 adults plus 3 children**. Children ride along for free: they are
  NOT part of your paid headcount, but you need to know they're coming.
- Guide chips = who runs this tour (co-guides included).
- "Not assigned" in italics = management hasn't staffed it yet — not an error.

## Booking rows

Tap a tour card to open its reservations: guest name, source badge (VIATOR /
GETYOURGUIDE / GURUWALK / …), phone (tap to call, WhatsApp icon to chat), and
the check-in controls.

## Checking guests in

1. When the group arrives, adjust the number with − / + if fewer adults came
   than booked (children don't count here).
2. Tap **Check in**. The button turns grey ✓ In and locks — done. Tapping
   twice or refreshing cannot double-save.
3. Walk-ins: use the Walk-in row (+ / −) at the bottom of the tour.
4. No signal? The app will tell you exactly whether it's your connection or
   the server, with a Retry button.

**If you do not check a reservation in, it counts as a no-show** after the
tour — for GuruWalk that means management won't pay commission for them, and
for Viator/GetYourGuide management reports them as no-shows. So check in
everyone who actually shows up, during or right after the tour.

## What you must NOT do

- Don't edit any Google Sheet directly — everything goes through the portal.
- Don't share your login or guests' phone numbers.

## Something looks wrong?

Wrong tour, wrong time, missing booking, can't log in → message Albert or
Carlos. Tell them the tour date, time and language you're looking at.
